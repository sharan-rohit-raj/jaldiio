  ...
  Widget build(BuildContext context) {
    ...
    return FlatButton(
      ...
      child: Text(
        "Delete",
        style:
            GoogleFonts.openSans(fontSize: 18, color: Colors.deepPurpleAccent),
      ),
      onPressed: () async {
...
//        print(codeController);
            if (familyCode.compareTo(codeController) == 0) {
              ...
              try {
                await DataBaseService(famCode: familyCode)
                    .deleteFamilyEventList();
              } catch (e) {
                print(e.toString());
                print("Unable to delete Events List");
              }
              try {
                await DataBaseService(famCode: familyCode).deleteFamily();
              } catch (e) {
                print(e.toString());
              ...
              value =
                  "Oh..Oh that's not what we are looking for. Please try again.";
              scaffoldKey.currentState
                  .showSnackBar(new SnackBar(content: new Text(value)));
            }
          }
